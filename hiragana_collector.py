...
def click_complete_button(event=None):
    global write_index
    img_save()
    if data_index >= data_limit and write_index == write_limit: # 마지막 종류의 마지막 반복까지 하면 끝내기
        complete()
        return
    write_index += 1
    update_write_index()
...
def update_data_index(): # 총 데이터 값 업데이트
    global data_index
    data_index += 1
    total_progress_text.set(f"{data_index + 1}/{data_limit + 1}")
...
def img_save():
    ...
    file_path = f'hiragana_data/{char_roman}'
    if not os.path.exists('hiragana_data'):
        os.mkdir('hiragana_data')
    ...
    resize_img.save(f"{file_path}/{char_roman}_{write_index}.png")
...
def progress_load():
    global data_index
    user_input = simpledialog.askstring("Load", "hiragana in roman")
